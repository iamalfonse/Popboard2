...
include("config.php");
	...
	if(!$likesRow2['user_id'] && ($likesUserhash == $likesRow['session_hash']) && isset($_COOKIE['login_cookie'])){
		...
		$likesQuery3 = @mysqli_query($dblink, "INSERT INTO likes(user_id,post_id, post_user_id) VALUES ('{$likesRow['user_id']}','$likesPostid','{$likesRow5['user_id']}')");
		...
		if(isset($_COOKIE['login_cookie'])){ //if loggeed in
			...
			$likesQuery3 = @mysqli_query($dblink, "DELETE FROM likes WHERE user_id='{$likesRow['user_id']}' AND likes.post_id = '$likesPostid';");
		...
		$likesQuery4 = @mysqli_query($dblink, "SELECT likes FROM posts WHERE id = '$likesPostid'");
		$likesNumrows4 = mysqli_fetch_assoc($likesQuery4);
		echo "<span>Likes</span> ".$likesNumrows4['likes'];
